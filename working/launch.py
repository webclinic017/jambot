...
if True:
    ...
    bm = bt.BacktestManager(**kw, strats=strat, df=df, partial=False)
    bm.decide_full()
    f.print_time(start)
    bm.print_final()
    bm.account.plot_balance(logy=False)
    trades = strat.trades
    t = trades[-1]
...
if True:
    ...
    syms = Parallel(n_jobs=-1)(delayed(op.run_single)(strattype, startdate, dfall,
                                                      speed[0], speed[1], row, norm) for row in dfsym.itertuples())

    results = [bm.result() for bm in syms]
    ...
    # for bm in syms:
    # 	bm.account.plot_balance(logy=True, title=bm.symbol)
...
if True:
    ...
    for row in dfsym[dfsym.symbol == symbol].itertuples():
        if True:
            ...
            strattype = 'trendrev'
            norm = (0.004, 0.024)
            syms = Parallel(n_jobs=-1)(delayed(op.run_single)(strattype, startdate, dfall, speed0,
                                                              speed1, row, norm) for speed0 in range(6, 23, 1) for speed1 in range(6, 19, 1))
        ...
        if False:
            ...
            mr = False
            opposite = False
            listout = Parallel(n_jobs=-1)(delayed(op.run_trend)(symbol, startdate, mr, df, against,
                                                                wth, row, titles) for against in range(8, 44, 2) for wth in range(8, 44, 2))
        ...
        if False:
            ...
            listout = Parallel(n_jobs=-1)(delayed(op.run_chop)(symbol, startdate, df,
                                                               speed[0], speed[1], tpagainst, tpwith, norm[0], norm[1], row, titles) for tpagainst in range(14, 45, 2) for tpwith in range(14, 45, 2))
        ...
        for i, bm in enumerate(syms):
            ...
            dfResults.loc[i] = [speed[0], speed[1], round(a.min, 3), round(
                a.max, 3), round(a.balance, 3), strat.tradecount()]
        ...
        ch.heatmap(dfResults, [titles[0], titles[1], 'max'],
                   title='{} - {} - {} - Max'.format(symbol, speed, startdate), dims=d)
        ch.heatmap(dfResults, [titles[0], titles[1], 'final'],
                   title='{} - {} - {} - Final'.format(symbol, speed, startdate), dims=d)
