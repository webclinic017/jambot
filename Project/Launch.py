...
if True:
	...
	# strat = c.Strat_TrendClose(speed=speed)
...
if True:
	...
	for row in dfsym[dfsym.symbol==symbol].itertuples():		
		...
		if False:
			...
			# norm
			# listout = Parallel(n_jobs=-1)(delayed(f.runchop)(symbol, startdate, df, speed[0], speed[1], speedtp[0], speedtp[1], lowernorm, uppernorm, row, titles) for lowernorm in range(1, 25) for uppernorm in range(1, 25))
...
if True:
	...
	# df = f.getDataFrame(symbol=symbol, startdate=f.startvalue(startdate), enddate=f.enddate(startdate, daterange))
	norm=(0.04, 0.24)
...
# cProfile.run('f.run_single(symbol=symbol,\
# 						strattype=strattype,\
# 						startdate=startdate,\
# 						dfall=df,\
# 						speed0=speed[0],\
...
cProfile.run('f.testdf(df)', filename = filename)

# cProfile.run('f.run_parallel()', filename=filename)
